...
<div class="container">
  <div class="card m-0">
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">Place Order <img class='buffer float-right d-none'
    ...
    <div class="card-body px-3 px-md-5">
      ...
      <div class="row">
        <div class="col-12 col-md-3">
          ...
          <div class="input-group ">
            ...
            <div class="input-group-append">
              <button class="btn btn-sm btn-primary rounded-right" onclick="CustomModal()"><i class="fas fa-plus"></i></button>
            </div>
        ...
        <div class="col-12 col-md-3">
          <div class="form-group">
            ...
            <select class="form-control form-control-sm" id="site">
            </select>
          </div>
        </div>
        <div class="col-12 col-md-3">
          <div class="form-group">
            <label class="font-weight-bold">Delivery Admin</label>
            <select class="form-control form-control-sm" id="delivery">
            </select>
          </div>
        </div>
      ...
      <table width="100%" class="table-sm table-bordered" id="sales-table">
        <thead>
          <tr>
            <th class="text-center" width="25%">{{__('key.invoice.invoice.table.product')}}</th>
            ...
            <th class="text-center" width="15%">Bundle</th>
            <th class="text-center" width="20%">{{__('key.invoice.invoice.table.total')}}</th>
            <th class="text-center" width="10%">{{__('key.invoice.invoice.table.action')}}</th>
          </tr>
        </thead>
        <tbody>
        </tbody>
...
<script type="text/javascript">
  ...
  $('#delivery').select2({
    ...
    allowClear:true,
    ajax:{
      url:"{{URL::to('admin/delivery/search')}}",
      type:'post',
      dataType:'json',
      delay:20,
      data:function(params){
...
function addItem(datx={id:null,text:null,qty:null,price:null}){
  ...
  var html='<tr>';
      ...
      html+="<td class='d-none'><input class='form-control form-control-sm text-right discount'  type='number' placeholder='0.00' name='discount[]' id='discount"+i+"'></td>";
      html+="<td><input class='form-control form-control-sm text-right bundle'  type='text' name='bundle[]' id='bundle"+i+"'></td>";
...
$('body').on('select2:select',"select[name='item[]']", function (e){
 ...
 axios.get('admin/product_price_by_id/'+id)
      .then(function(response){
            this_cat.parent().next().next().children("[name='price[]']").val(response.data);
            calculation();
          })
...
if (isValid==true) {
  $('.submit').attr('disabled',true);
   ...
   pay=$('#pay').val();
    ...
    axios.post('admin/create-order',formData)
    .then(function(response){
    ...
    <table width="100%" style="border:none;">
      <tr>
        <td>
          ...
          <span style="font-size:25px;">Order Invoice</span>
        </td>
  ...
  <div style="margin-right:50px;margin-left:50px;margin-top:30px;margin-bottom:30px;">
    <table width="100%" style="border:none;font-weight:bold;">
      <tr>
        ...
        <td style="float:right;">Order Place</td>
      </tr>
      <tr>
        <td>
