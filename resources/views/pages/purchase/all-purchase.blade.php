...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">@lang('key.purchase.purchase_list.title')</h5>
     </div>
    <div class="card-body px-3 px-md-5">
        <!-- Modal --> 
        <a class="btn btn-primary text-light font-weight-bold mb-2" href="{{URL::to('admin/purchase')}}">
          @lang('key.purchase.purchase_list.add_new')<i class="ml-1 fas fa-plus"></i>
        </a>
        <a class="btn btn-primary text-light font-weight-bold mb-2 float-right" href="{{URL::to('admin/purchase')}}">
          @lang('key.purchase.purchase_list.add_return')<i class="ml-1 fas fa-plus"></i>
        </a>
        <table class="table table-sm text-center table-bordered table-striped data-table">
          <thead>
            <tr>
              ...
              <th>@lang('key.purchase.purchase_list.supplier_name')</th>
              <th>@lang('key.purchase.purchase_list.type')</th>
              <th>@lang('key.purchase.purchase_list.total_payable')</th>
              <th>@lang('key.purchase.purchase_list.total')</th>
...
.then((isConfirmed) => {
  ...
  var id=$(this).data('id');
    axios.delete('/admin/purchase/'+id,{_method:'DELETE'})
      .then((res)=>{
        if (res.data.message) {
