...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">Stock Summery</h5>
     </div>
...
function Request(){
  ...
  toDate=$('#toDate').val()
  product=$('#product').val();
  axios.post('/admin/stock_summery',{fromDate:fromDate,toDate:toDate,product:product})
  .then((res)=>{
    console.log(res)
    if (res.data.errors) {
    ...
    let html='';
      ...
      for (var i = 0; i < data.length; i++){
      	if(data[i]['name']=='Purchase' || data[i]['name']=='Sale Return' ||data[i]['name']=='Prev-Stock' || data[i]['name']=='Opening-Stock' ){
          total_deb_qty+=parseFloat(data[i]['qty']);
        }
        if(data[i]['name']=='Purchase Return' || data[i]['name']=='Sale' || data[i]['name']=='Damage-Out'){
          total_cred_qty+=parseFloat(data[i]['qty']);
        }
        if(data[i]['qty']!=0.00){
      ...
      html+=`<tr style='height:12px;font-weight:bold;'>
                <td></td>
                <td>Current Stock: </td>
                <td>`+(total_deb_qty-total_cred_qty).toFixed(2)+`</td>
             </tr>`
       html+="</tbody>"
      header=`<h5 style='text-align:center;margin-top:30px;line-height:0.6'>Stock Summery</h5>
