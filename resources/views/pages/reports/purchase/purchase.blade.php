...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">@lang('key.purchase_summary.title')</h5>
     </div>
    <div class="card-body px-3 px-md-5">
      <form>
        <div class="input-group">
         <label class="control-label col-sm-2 text-lg-right" for="type">@lang('key.purchase_summary.type') :</label>
         <div class="col-sm-9">
            <select class="form-control form-control-sm" name="type" id="type">
            <option value="0">@lang('key.purchase_summary.normal_purchase')</option>
            <option value="1">@lang('key.purchase_summary.advance_purchase')</option>
            <option value="2">@lang('key.purchase_summary.purchase_return')</option>   
          </select>
       ...
       <div class="row ml mt-1">       
        <label class="col-sm-2 text-lg-right">@lang('key.purchase_summary.date') :</label>
        <div class="input-group input-group-sm col-sm-9" id="date">
            <div class="input-group-prepend">
              <span class="input-group-text" id="inputGroup-sizing-sm">@lang('key.purchase_summary.from') :</span>
            </div>
            <input class="form-control form-control-sm" name="fromDate" id="fromDate">
            <div id="fromDate_msg" class="invalid-feedback">
             </div>
            <div class="input-group-prepend">
              <span class="input-group-text" id="inputGroup-sizing-sm">@lang('key.purchase_summary.to') :</span>
            </div>
        ...
        <div class="col-md-2" id="submit">
          <button class="btn btn-sm btn-primary" onclick="Request()">@lang('key.purchase_summary.create_report')</button>
        </div>
            </div>
  </div>
</div>
...
function Request(){
  ...
  toDate=$('#toDate').val()
  type=$('#type').val()
  axios.post('/admin/purchase_summery',{fromDate:fromDate,toDate:toDate,type:type})
  .then((res)=>{
    if (res.data.errors) {
    ...
    let html='';
      ...
      for (var i = 0; i < data.length; i++){
        html+=`<tr style='height:12px;'>
                ...
                <td>`+parseFloat(data[i]['qantity'])+`</td>
                <td>`+parseFloat(data[i]['price']).toFixed(2)+`</td>
                <td>`+parseFloat(data[i]['price'])*parseFloat(data[i]['qantity'])+`</td>
               </tr>`
      ...
      header=`<h5 style='text-align:center;margin-top:30px;line-height:0.6;'>@lang('key.purchase_summary.title')</h5>
