...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">Earn Pay Statement <img class="float-right buffer d-none" src="{{asset('storage/admin-lte/dist/img/buffer.gif')}}" alt=""></h5>
     </div>
...
function Request(){
  ...
  toDate=$('#toDate').val()
  axios.post('/admin/earn_pay_statement',{fromDate:fromDate,toDate:toDate})
  .then((res)=>{
    data=res.data.get;
    let html='';
       html+='<table>'
      ...
      for (var i = 0; i < data.length; i++){
      	if(data[i]['name']=='Previous Balance' || data[i]['name']=='Income'){
      		earn+=parseFloat(data[i]['total']);
      	}
      	if(data[i]['name']=="Pay"){
      		pay=parseFloat(data[i]['total']);
      	}
       ...
       html+=`<tfoot>
              <tr>
                <th colspan="2"></th>
                <th style='text-align:right;font-size:12px;'>Balance: `+(earn-pay).toFixed(2)+`</th>
              </tr>
            </tfoot>`;
      header=`<h6 style='text-align:center;margin-top:25px;'>Earn Pay Statement</h6>
