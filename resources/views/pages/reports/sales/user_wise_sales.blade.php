...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">@lang('key.user_wise_sale.title')</h5>
      </div>
    <div class="card-body px-3 px-md-5">
      <form>
      <div class="input-group">
         <label class="control-label col-sm-2 text-lg-right" for="type">@lang('key.user_wise_sale.user') :</label>
         <div class="col-sm-9">
        ...
        <div class="input-group mt-2">
         <label class="control-label col-sm-2 text-lg-right" for="type">@lang('key.user_wise_sale.sale_type') :</label>
         <div class="col-sm-9">
            <select class="form-control form-control-sm" name="type" id="type">
            <option value="0">@lang('key.user_wise_sale.normal_sale')</option>
            <option value="1">@lang('key.user_wise_sale.advance_sale')</option>
            <option value="2">@lang('key.user_wise_sale.sale_return')</option>
            <option value="3">@lang('key.user_wise_sale.installment')</option>
          </select>
             <div id="type_msg" class="invalid-feedback">
             </div>
          </div>
       ...
       <div class="row ml mt-1">       
        <label class="col-sm-2 text-lg-right">@lang('key.user_wise_sale.date') :</label>
        <div class="input-group input-group-sm col-sm-9" id="date">
            <div class="input-group-prepend">
              <span class="input-group-text" id="inputGroup-sizing-sm">@lang('key.user_wise_sale.from') :</span>
            </div>
            <input class="form-control form-control-sm" name="fromDate" id="fromDate">
            <div id="fromDate_msg" class="invalid-feedback">
             </div>
            <div class="input-group-prepend">
              <span class="input-group-text" id="inputGroup-sizing-sm">@lang('key.user_wise_sale.to') :</span>
            </div>
        ...
        <div class="col-md-2" id="submit">
          <button class="btn btn-sm btn-primary" onclick="Request()">@lang('key.user_wise_sale.create_report')</button>
        </div>
            </div>
  </div>
</div>
...
function Request(){
  ...
  type=$('#type').val()
  user=$('#user').val()
  axios.post('/admin/user_wise_sales',{fromDate:fromDate,toDate:toDate,type:type,user:user})
  .then((res)=>{
    console.log(res)
    if (res.data.errors) {
    ...
    let html='';
      ...
      header=`<h5 style='text-align:center;margin-top:30px;line-height:0.6'>@lang('key.user_wise_sale.title')</h5>
               <h6 style='text-align:center;line-height:0.6;'>`+$('#type option:selected').text()+`</h6>
               <span style='text-align:center;line-height:1.2;'><strong>@lang('key.report-common.username') : </strong>`+$('#user option:selected').text()+`</span>
      ...
      footer=`<div><p style='text-align:center;font-size:10px;'>Developed by SOFTiMPIRE || 01715279498</p></div>`
