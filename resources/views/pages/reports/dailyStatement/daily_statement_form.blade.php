...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">@lang('key.daily_statement.title') <img class="float-right buffer d-none" src="{{asset('storage/admin-lte/dist/img/buffer.gif')}}" alt=""></h5>
     </div>
...
function Request(){
  ...
  .then((res)=>{
    ...
    let html='';
       ...
       html+="<th width='10%'>@lang('key.report-common.bank_name')</th>"
       html+="<th width='15%'>@lang('key.report-common.category')</th>"
      ...
      for (var i = 0; i < data.length; i++){
        html+=`<tr style='height:12px;'>
                <td>`+i+`</td>                
                <td>`+'1'+String(data[i]['id']).padStart(9,'0')+`</td>
       ...
       html+=`<tfoot>
              <tr>
                <th colspan="6"></th>
                <th style='text-align:right;font-size:12px;'>@lang('key.report-common.total'): `+deposit.toFixed(2)+`</th>
                <th style='text-align:right;font-size:12px;'>@lang('key.report-common.total'): `+expence.toFixed(2)+`</th>
              </tr>
                <th colspan='8' style='text-align:right;font-size:12px;'>@lang('key.report-common.grand_total') : `+(deposit-expence).toFixed(2)+`</th>
              <tr>
              </tr>
            </tfoot>`;
      header=`<h6 style='text-align:center;margin-top:25px;'>@lang('key.daily_statement.title')</h6>
