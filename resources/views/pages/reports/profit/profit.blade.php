...
<div class="container">
    ...
    <div class="card-header pt-3  flex-row align-items-center justify-content-between">
      <h5 class="m-0 font-weight-bold">@lang('key.profit_loss.title')<img class="float-right buffer d-none" src="{{asset('storage/admin-lte/dist/img/buffer.gif')}}" alt=""></h5>
     </div>
    <div class="card-body px-3 px-md-5">
      <form>
        <div class="input-group input-group-sm col-md-4 col-12" id="date">
          <div class="input-group-prepend">
            <span class="input-group-text" id="inputGroup-sizing-sm">@lang('key.profit_loss.from')</span>
          </div>
          <input class="form-control" name="fromDate" id="fromDate">
          <div class="input-group-prepend">
            <span class="input-group-text" id="inputGroup-sizing-sm">@lang('key.profit_loss.to')</span>
          </div>
          <input class="form-control" name="toDate" id="toDate">
        </div>
        </form>
        <div class="col-md-2" id="submit">
          <button class="btn btn-sm btn-primary" onclick="Request()">@lang('key.profit_loss.create_report')</button>
        </div>
...
function Request(){
  ...
  toDate=$('#toDate').val()
  axios.post('/admin/profit_loss',{fromDate:fromDate,toDate:toDate})
  .then((res)=>{
    data=res.data.get;
    let html='';
       html+='<table>'
       ...
       html+="<th width='5%'>@lang('key.report-common.no')</th>"
       html+="<th width='20%'>@lang('key.report-common.product_name')</th>"
       html+="<th width='10%'>@lang('key.report-common.qantity')</th>"
       html+="<th width='10%'>@lang('key.report-common.sale_rate')</th>"
       html+="<th width='10%'>@lang('key.report-common.purchase_rate')</th>"
       html+="<th width='15%'>@lang('key.report-common.total_sale_rate')</th>"
       html+="<th width='15%'>@lang('key.report-common.total_purchase_rate')</th>"
       html+="<th width='15%'>@lang('key.profit_loss.profit')</th>"
       html+='</tr>'
       html+="</thead>"
      ...
      if(total>=0){
        label="@lang('key.profit_loss.profit')";
      }else{
        label="@lang('key.profit_loss.loss')"
      }
       html+="</tbody>"
       html+=`<tfoot>
              <tr>
                <th colspan="6"></th>
                <th colspan='2' style='text-align:right;font-size:12px;'>`+label+': '+total.toFixed(2)+`</th>
              </tr>
              <tr>
              </tr>
            </tfoot>`;
      header=`<h6 style='text-align:center;margin-top:25px;'>@lang('key.profit_loss.title')</h6>
