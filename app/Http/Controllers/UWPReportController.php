...
use Validator;
use DB;
class UWPReportController extends Controller
{
    public function __construct(){
        $this->middleware('auth');
    }
    public function Form(){
        return view('pages.reports.purchase.user_wise_purchase');
    }
        ...
        if($validator->passes()){
            ...
            SELECT purchases.dates,products.product_name,sum(purchases.deb_qantity)+(purchases.cred_qantity) qantity,purchases.price,purchases.invoice_id
             from purchases
                ...
                where purchases.user_id=:id and purchases.dates >=:fromDate and purchases.dates <= :toDate
