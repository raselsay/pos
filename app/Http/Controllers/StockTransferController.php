    ...
    public function AllStockTransfer() {
        if (request()->ajax()) {
            $get = DB::table('invpurchases')
                ...
                ->where('invpurchases.action_id',4)
        ...
        return view('pages.purchase.all_stock_transfer');
    }
    public function Delete($id){
        ...
        if ($data['action_id']==4) {
            ...
            if ($data){
                ...
                $notification->details = 'Stock Transfer No <strong>' . $id . '</strong>' . ' deleted by <strong>' . Auth::user()->name . '(' . Auth::user()->id . ')</strong>';
                $notification->action = 'delete';
                $save = $notification->save();
                if ($save) {
                    return response()->json(['message' => 'Stock Transfer Deleted Success']);
                }
            }
        }
    }
